...
public class WaitingPlayersFormController {
    ...
    private static final int WAIT_TIME_LIMIT_SECONDS = 35;
    ...
    public void showAction(WindowEvent event) {
        ...
        for (int i = 0; i < size; ) {
            avatarBlocks[i] = (ImageView) nodes.get(i++);
            labelBlocks[i - 1] = (Text) nodes.get(i++);
        }
        updateInfo();
        updateTimeFields();
        playCountdown();
    ...
    private void updateInfo() {
        ...
        try {
            ...
            for (Player player : players) {
                ...
                // FIXME: 26.11.2018 тут выбрасывается исключение, пофиксить
                i++;
            }
        } catch (Exception e) {
            e.printStackTrace();
